        ...
        IntStream.range(0, 100_000).forEach(i -> {
            Runnable task = () -> {
                if (isPalindrome(texts[i])) {
                    if (texts[i].length() == 3) {
                        executorService1.submit(atomicInt1::incrementAndGet);
                    } else if (texts[i].length() == 4) {
                        executorService1.submit(atomicInt2::incrementAndGet);
                    } else if (texts[i].length() == 5) {
                        executorService1.submit(atomicInt3::incrementAndGet);
                    }
                }
            };
